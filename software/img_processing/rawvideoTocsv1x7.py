...
def addframe2data(frame, databins, bz_coord, col):
    ...
    for i in range(7):
        ...
        for j in range(1):
            ...
            pointA = (x1+pad+30 + stepw*i, y1 + 40 + steph * j)
            # bottom right corner of the current cell (i,j)
            pointB = (x1+pad+80 + stepw*i, y1 + -40 + steph * (j+1))
...
def generate_singlevideo_csv(path, processLimiter=multiprocessing.Lock()):
    ...
    with processLimiter:
        ...
        click_grid = GridClickData17() # to store the 5x5 grid coordinates
        ...
        databins = np.zeros((7, total_frames), np.uint8)
